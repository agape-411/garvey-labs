  ...
  return (
    <section className="bg-white py-20 px-6 border-t border-gray-200">
      <div className="max-w-5xl mx-auto">
        ...
        {/* 2-column layout with horizontal dividers and top borders */}
        <div className="flex flex-col md:flex-row md:space-x-12">
          {[servicesList.slice(0, 2), servicesList.slice(2)].map(
            (group, groupIdx) => (
              <div
                key={groupIdx}
                className="flex-1 space-y-8 md:border-t md:border-gray-600 pt-8"
              >
                {group.map((service, idx) => (
                  <div
                    key={idx}
                    className="border-b border-gray-600 pb-8 "
                  >
                    ...
                    <Link
                      href={service.href}
                      className="text-sm md:text-base font-medium text-blue-700 hover:underline underline-offset-4 inline-block mt-3"
                    >
                      Learn More &rarr;
                    </Link>
                  </div>
